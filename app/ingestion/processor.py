...
def process_file(file_path: str, filename:str, source_type: str):
    ...
    with logfire.span("Processing File: ", filename=filename,source=source_type):
        try:
            ...
            if ext == "pdf":
                ...
                logfire.warning(f"skipping Unsupported source type: {source_type}")
                return
            
            if not full_text.strip():
                logfire.warning(f"No text extracted from {file_path}. Skipping further processing.")
                return
            ...
            if not chunks:
                logfire.warning(f"No chunks created from {file_path}. Skipping further processing.")
                return
            ...
            with logfire.span("vectorizing and indexing", filename=filename, source=source_type):
                # 4. Embed
                ...
                logfire.info(f"Indexed {len(points)} chunks from {filename} into Qdrant.")
